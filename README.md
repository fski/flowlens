...
- **Auto-capture** — once you press **Record Flow**, steps are captured automatically as you navigate, including SPA route changes (History API). Third-party origins (SSO logins, payment gateways) are skipped for privacy — auto-captured steps include viewport screenshots, and those pages routinely show credentials or card data. Manual **Mark step** stays for inserting a step by hand (works on a third-party page too — that's a deliberate action); the **Auto** toggle opts out.
